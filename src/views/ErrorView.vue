<template>
  <div
    class="items-center max-w-[100vw] flex justify-center flex-col lg:gap-16 lg:flex-row gap-8 pb-8 px-6"
  >
    <p class="text-center font-extrabold text-9xl lg:text-[14em] select-none">
      {{ statusCode ?? 404 }}
    </p>
    <div class="text-center lg:text-left w-96 max-w-full flex flex-col gap-8">
      ...
      <LinkShadowButton
        to="/"
        color="black"
        >Back to Homepage</LinkShadowButton
      >
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    statusCode() {
      var statusEl = document.querySelector('#app') as HTMLElement;
      if (statusEl) {
